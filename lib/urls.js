const API_HOST = process.env.NEXT_PUBLIC_API_HOST;

export const urls = {
  login: `${API_HOST}/api/v2/auth/login`,
  getAllEvents: `${API_HOST}/api/v2/events`,
  getRegistration: (eventId, ticketIdOrEmail) =>
    `${API_HOST}/api/v2/events/${eventId}/registrations/${ticketIdOrEmail}`,
  checkIn: (ticketId) => `${API_HOST}/api/v2/tickets/${ticketId}`,

  // Analytics endpoints
  analytics: {
    eventStats: (eventId) =>
      `${API_HOST}/api/v2/analytics/events/${eventId}/stats`,
    leaderboard: `${API_HOST}/api/v2/analytics/leaderboard`,
    summary: `${API_HOST}/api/v2/analytics/summary`,
    userCheckIns: (userId) =>
      `${API_HOST}/api/v2/analytics/users/${userId}/check-ins`,
  },

  // Bulk email endpoints
  tickets: {
    bulkSend: `${API_HOST}/api/v2/tickets/bulk/send`,
    bulkSendByEvent: `${API_HOST}/api/v2/tickets/bulk/send-by-event`,
    bulkResend: `${API_HOST}/api/v2/tickets/bulk/resend`,
    bulkStatus: (jobId) => `${API_HOST}/api/v2/tickets/bulk/status/${jobId}`,
  },

  // Registrations endpoints
  registrations: {
    byEvent: (eventId) => `${API_HOST}/api/v2/events/${eventId}/registrations`,
  },

  // Users endpoints
  users: {
    list: `${API_HOST}/api/v2/users`,
    create: `${API_HOST}/api/v2/users`,
  },
};
