...
const ResultContainer = ({result, emailOrTicketId}) => {
        ...
        return (
            <div className="flex justify-center items-center h-full">
                <div className="text-center">
                    ...
                    <p className="text-lg text-gray-600">{result.error.context}</p>
                </div>
            </div>
        );
    }
    ...
    function handleCheckin() {
        setIsLoading(true);
        fetch(`${process.env.NEXT_PUBLIC_API_HOST}/events/${process.env.NEXT_PUBLIC_EVENT_ID}/registrations/${ticketId}`, {
            method: "POST",
            headers: {
                    ...
                    <div className="space-y-1">
                        ...
                        { result.data.tickets.length > 1 && result.data.tickets.filter((e)=>e.status === "Pending").length > 0 && (
                            <div className="border border-dotted rounded p-4 !mt-5">
                                ...
                                {!checkedIn ? (
                                    <button className="btn !bg-green-500 my-5" onClick={handleCheckin} disabled={isLoading}>
                                        {isLoading ? (
                                    ...
                                    <button className="btn !bg-green-500 my-5">
                                        <div className="flex justify-center" role="status">
                                            <svg className="w-8 h-8 text-white rotate-12" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                ...
                <div className="text-center">
                    {checkedIn ? (
                        <>
                            ...
                            <p className="text-lg text-gray-600">Ticket has been checked in successfully!</p>
                        </>
                    ) : (
                        <>
                            <svg className="mx-auto w-10 h-10 text-green-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                            ...
                            <p className="text-lg text-gray-600">Please confirm the check in for this ticket</p>
                        </>
                    )}
                    <div>
                        <hr className="my-5 w-[80vw]" />
